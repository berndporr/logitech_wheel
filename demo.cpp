...
{
  public:
    void steering (float v) { printf ("Steering = %f\n", v); }

    void throttle (float v) { printf ("Throttle = %f\n", v); }

    void braking (float v) { printf ("Braking = %f\n", v); }

    void button (int d) { printf ("Button = %d\n", d); }
};
...
int main (int argc, char *argv[])
{
    LogiWheel logiwheel;
    Printer printer;
    logiwheel.registerSteeringCallback (
        [&] (float v) { printer.steering (v); });
    logiwheel.registerThrottleCallback (
        [&] (float v) { printer.throttle (v); });
