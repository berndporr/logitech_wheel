...
class RemotePublisher
{
  ...
  public:
    ...
    bool init ()
    {
        ...
        if (topicSteering == nullptr)
            return false;
        writerSteering = publisher->create_datawriter (topicSteering,
        ...
        if (topicThrottle == nullptr)
            return false;
        writerThrottle = publisher->create_datawriter (topicThrottle,
                                                       DATAWRITER_QOS_DEFAULT);
        if (writerThrottle == nullptr)
            return false;
        ...
        if (topicBrake == nullptr)
            return false;
        writerBrake = publisher->create_datawriter (topicBrake,
                                                    DATAWRITER_QOS_DEFAULT);
        if (writerBrake == nullptr)
            return false;
        ...
        if (topicButton == nullptr)
            return false;
        writerButton = publisher->create_datawriter (topicButton,
                                                     DATAWRITER_QOS_DEFAULT);
        if (writerBrake == nullptr)
            return false;
    ...
    void publishSteering (SteeringMsg &msg) { writerSteering->write (&msg); }

    void publishThrottle (ThrottleMsg &msg) { writerThrottle->write (&msg); }

    void publishBrake (BrakeMsg &msg) { writerBrake->write (&msg); }

    void publishButton (ButtonMsg &msg) { writerButton->write (&msg); }
};
...
int main (int argc, char *argv[])
{
    ...
    if (!logiwheel.start ())
    {
        std::cerr << "Wheel init fail. Is it plugged in? Exiting."
                  << std::endl;
        return -1;
    }
