...
class LogiWheel
{
  ...
  public:
    ...
    void registerBrakeCallback (AnalogueCallback cb) { brakeCallback = cb; }

    void registerButtonCallback (ButtonCallback cb) { buttonCallback = cb; }
        ...
        while (running)
        {
            ...
            if (r > 0)
            {
                ...
                if (event.type == EV_ABS && event.code < ABS_TOOL_WIDTH)
                {
                    ...
                    case 0:
                        if (steeringCallback)
                            steeringCallback ((v - 512) / 512.0);
                        break;
                    case 1:
                        if (throttleCallback)
                            throttleCallback (1 - v / 256.0);
                        break;
                    case 2:
                        if (brakeCallback)
                            brakeCallback (1 - v / 256.0);
                        break;
                    case 16:
                        if (buttonCallback)
                            switch (event.value)
